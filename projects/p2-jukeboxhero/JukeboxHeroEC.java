...
/**
 * JukeboxHero class acts as a song catalog manager. Extra credit version
 *
 * @author Brady Ward
...
public class JukeboxHeroEC {
    /**
     * Main method of JukeboxHeroEC class. It creates a UI to allow someone to interact with
     * User can input:
     * - l to load a file
    ...
    public static void main(String args[]) { //TODO add method calls for every case, not directly in main for EC
        ...
        while (true) {
            ...
            switch (keyboard.nextLine().toLowerCase()) {
                ...
                case "l":
                    loadFile(songList, keyboard);
                    break;

                case "s":
                    searchCatalog(songList, keyboard);
                    break;
    ...
    /**
     * Function to load files in
     * 
     * @param sl
     * @param kbd
     */
    private static void loadFile(ArrayList<Song> sl, Scanner kbd) {
        // Collect file name and open it
        System.out.println("Enter a file name to load");
        File musicFile = new File(kbd.nextLine());
        try {
    ...
    /**
     * Function to search through all songs
     * @param sl
     * @param kbd
     */
    private static void searchCatalog(ArrayList<Song> sl, Scanner kbd) {
        ...
        String searchTerm = kbd.nextLine();
    ...
    /**
     * Function to analyze the catalog and give user certain info about it
     * @param sl
     */
    ...
    /**
     * Function to print out all songs and related info that ar ein the catalog
     * @param sl
     */
    private static void printCatalog(ArrayList<Song> sl) {
        ...
        System.out.println("Playlist has " + sl.size() + " total songs ");
        System.out.println("-".repeat(30));
        for (Song song : sl) {
